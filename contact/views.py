    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Send email
                send_mail(subject, message, 'fineshine@example.com', ['brunarihl@gmail.com'])

                return redirect('success')
            except Exception as e:
                # Log the error or handle it accordingly
                print(f"Error sending email: {e}")
                return render(request, 'contact.html', {'form': form, 'error': 'There was an error sending your message. Please try again later.'})
    else:
        form = ContactForm()
